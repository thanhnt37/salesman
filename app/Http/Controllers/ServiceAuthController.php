...
class ServiceAuthController extends Controller
{
    ...
    public function callback()
    {
        ...
        try {
            $serviceUser = $this->socialite->driver( $this->driver )
                                           ->user();
        } catch( \GuzzleHttp\Exception\ClientException $e ) {
            return redirect()
                ->action( $this->errorRedirectAction )
                ->withErrors( [trans( 'sign_in_failed_title' ), trans( 'social_sign_in_failed' )] );
        }
        ...
        if( empty( $email ) ) {
            return redirect()
                ->action( $this->errorRedirectAction )
                ->withErrors( [trans( 'sign_in_failed_title' ), trans( 'failed_to_get_email' )] );
        }
