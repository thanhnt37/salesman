...
class PaginationHelper implements PaginationHelperInterface
{
    ...
    )
    {
        ...
        $data[ 'previousPageLink' ] = $page <= 1 ? '' : $this->generateLink(
            $page - 1,
            $path,
            $query,
            $order,
        ...
        $data[ 'nextPageLink' ] = $page >= $lastPage ? '' : $this->generateLink(
            $page + 1,
            $path,
            $query,
            $order,
    ...
    private function generateLink( $page, $path, $query, $order, $direction )
    {
        return $path . '?' . http_build_query(
            array_merge( $query, ['page' => $page, 'order' => $order, 'direction' => $direction] )
        );
    }
}
